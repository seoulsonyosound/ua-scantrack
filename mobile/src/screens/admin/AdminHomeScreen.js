...
export function AdminHomeScreen({ navigation }) {
    ...
    Animated.parallel([
      ...
      Animated.loop(
        Animated.sequence([
          ...
          Animated.timing(shapeRotation, { toValue: 0, duration: 3000, easing: Easing.inOut(Easing.sin), useNativeDriver: true })
        ])
      )
    ]).start();
  }, []);
  ...
  const NavTile = ({ label, title, screen, color = "#4f46e5" }) => {
    ...
    return (
      ...
      >
        ...
        <View style={{ zIndex: 2 }}>
          ...
          <Animated.View style={{ height: 3, backgroundColor: color, marginTop: 8, borderRadius: 2, width: revealAnim.interpolate({ inputRange: [0, 1], outputRange: [25, 60] }) }} />
        </View>
      </Pressable>
    );
  };
      ...
      <ScrollView style={{ flex: 1, zIndex: 1 }} contentContainerStyle={{ padding: SCREEN_WIDTH > 600 ? 40 : 20 }}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: 30 }}>
          <View style={{ flexShrink: 1, marginRight: 10 }}>
            ...
            <Text style={[styles.kicker, { letterSpacing: 4, color: '#e9971c' }]}>ADMINISTRATOR</Text>
            <Text style={[styles.heroTitle, { fontSize: SCREEN_WIDTH > 600 ? 48 : 36, fontWeight: '900', color: '#06143be1' }]}>Dashboard</Text>
          </View>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' }}>
          <NavTile label="Scanner" title="Quick Scan" screen="QuickScan" color="#b91c1c" />
          <NavTile label="Events" title="Manage All" screen="ManageEvents" color="#2d52b6" />
          <NavTile label="Attendance" title="Records" screen="ManageAttendance" color="#047857" />
          <NavTile label="Data" title="Reports (CSV)" screen="AdminReports" color="#b45309" />
          <NavTile label="Users" title="Manage Students" screen="ManageStudents" color="#6d6d6d" />
          
          {/* Fillers for grid alignment */}
          <View style={{ width: SCREEN_WIDTH > 768 ? '31%' : '48%' }} /> 
        </View>
      </ScrollView>
    </SafeAreaView>
  );
